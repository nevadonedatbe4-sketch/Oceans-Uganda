...
const DEFAULT_SIDEBAR_IMG =
  'https://readdy.ai/api/search-image?query=professional%20real%20estate%20agent%20smiling%20friendly%20Kampala%20Uganda%20portrait%20business%20attire%20modern%20office%20background%20confident%20warm%20approachable%20person&width=480&height=640&seq=home-contact-agent&orientation=portrait';
...
export default function ContactSection() {
  ...
  const whatsappDirect =
    get('whatsapp_direct_url') ||
    `https://wa.me/${(whatsapp || phone).replace(/\D/g, '')}`;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch('https://readdy.ai/api/form/d76k5ivlpdqvlupfq0vg', {
        method: 'POST',
  ...
  return (
    ...
    >
      ...
      {isAdmin && (
        <Link
          to="/admin/management/contact"
          className="absolute top-4 right-4 z-50 flex items-center gap-2 bg-white/90 backdrop-blur-sm text-stone-800 text-xs font-semibold px-3 py-2 rounded-lg hover:bg-white transition-all cursor-pointer border border-stone-200 whitespace-nowrap"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-16 items-start">
          ...
          >
            ...
            {(whatsapp || phone) && (
              <div className="flex items-center gap-2 mb-2">
                <div className="w-5 h-5 flex items-center justify-center">
                  <i className="ri-whatsapp-line text-base" style={{ color: GOLD }} />
                </div>
                <a
                  href={whatsappDirect}
                  target="_blank"
            ...
            {email && (
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 flex items-center justify-center">
                  <i className="ri-mail-line text-base" style={{ color: GOLD }} />
                </div>
                <a
                  href={`mailto:${email}`}
                  className="font-roboto text-sm text-stone-600 hover:text-primary transition-colors cursor-pointer"
          ...
          <div className="md:col-span-2 pb-6 md:pb-8">
            <div className="mb-4 md:mb-6">
              <h2
                className="font-prata text-primary mb-1 md:mb-2"
                style={{ fontSize: 'clamp(1.75rem, 4vw, 2.75rem)' }}
              >
                Connect with Us
              </h2>
              <p
                className="font-roboto text-[10px] sm:text-xs md:text-sm font-semibold uppercase tracking-[0.2em] sm:tracking-[0.3em] md:tracking-[0.4em] whitespace-normal sm:whitespace-nowrap"
                style={{ color: GOLD }}
              >
            ...
            >
              {submitted ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  >
                    <i className="ri-check-double-line text-3xl" style={{ color: GOLD }} />
                  </div>
